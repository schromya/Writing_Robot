...
#            return ST
    ...
    property v:
        def __get__ (self):
            vector_size = self.thisptr.v.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.v[i])) for i in range(vector_size)]
            return prop

    property a:
        def __get__ (self):
            vector_size = self.thisptr.a.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.a[i])) for i in range(vector_size)]
            return prop


    property mJoints:
        def __get__ (self):
            vector_size = self.thisptr.mJoints.size()
            prop = [Joint (address=<uintptr_t> &(self.thisptr.mJoints[i])) for i in range(vector_size)]
            return prop

    property S:
        def __get__ (self):
            vector_size = self.thisptr.S.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.S[i])) for i in range(vector_size)]
            return prop

    property v_J:
        def __get__ (self):
            vector_size = self.thisptr.v_J.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.v_J[i])) for i in range(vector_size)]
            return prop

    property c_J:
        def __get__ (self):
            vector_size = self.thisptr.c_J.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.c_J[i])) for i in range(vector_size)]
            return prop
    ...
    property X_T:
        def __get__ (self):
            vector_size = self.thisptr.X_T.size()
            prop = [SpatialTransform (address=<uintptr_t> &(self.thisptr.X_T[i])) for i in range(vector_size)]
            return prop
    ...
    property c:
        def __get__ (self):
            vector_size = self.thisptr.c.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.c[i])) for i in range(vector_size)]
            return prop

    property IA:
        def __get__ (self):
            vector_size = self.thisptr.IA.size()
            prop = [SpatialMatrix (address=<uintptr_t> &(self.thisptr.IA[i])) for i in range(vector_size)]
            return prop

    property pA:
        def __get__ (self):
            vector_size = self.thisptr.pA.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.pA[i])) for i in range(vector_size)]
            return prop

    property U:
        def __get__ (self):
            vector_size = self.thisptr.U.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.U[i])) for i in range(vector_size)]
            return prop
    ...
    property f:
        def __get__ (self):
            vector_size = self.thisptr.f.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.f[i])) for i in range(vector_size)]
            return prop

    property I:
        def __get__ (self):
            vector_size = self.thisptr.I.size()
            prop = [SpatialRigidBodyInertia (address=<uintptr_t> &(self.thisptr.I[i])) for i in range(vector_size)]
            return prop

    property Ic:
        def __get__ (self):
            vector_size = self.thisptr.Ic.size()
            prop = [SpatialRigidBodyInertia (address=<uintptr_t> &(self.thisptr.Ic[i])) for i in range(vector_size)]
            return prop

    property hc:
        def __get__ (self):
            vector_size = self.thisptr.hc.size()
            prop = [SpatialVector (address=<uintptr_t> &(self.thisptr.hc[i])) for i in range(vector_size)]
            return prop


    property X_lambda:
        def __get__ (self):
            vector_size = self.thisptr.X_lambda.size()
            prop = [SpatialTransform (address=<uintptr_t> &(self.thisptr.X_lambda[i])) for i in range(vector_size)]
            return prop

    property X_base:
        def __get__ (self):
            vector_size = self.thisptr.X_base.size()
            prop = [SpatialTransform (address=<uintptr_t> &(self.thisptr.X_base[i])) for i in range(vector_size)]
            return prop


    property mFixedBodies:
        def __get__ (self):
            vector_size = self.thisptr.mFixedBodies.size()
            prop = [FixedBody (address=<uintptr_t> &(self.thisptr.mFixedBodies[i])) for i in range(vector_size)]
            return prop
    ...
    property mBodies:
        def __get__ (self):
            vector_size = self.thisptr.mBodies.size()
            prop = [Body (address=<uintptr_t> &(self.thisptr.mBodies[i])) for i in range(vector_size)]
            return prop
...
cdef class _InverseKinematicsConstraintSet_body_points_Vector3d_VectorWrapper:
    cdef crbdl.InverseKinematicsConstraintSet *parent

    def __cinit__ (self, uintptr_t ptr):
        self.parent = <crbdl.InverseKinematicsConstraintSet *> ptr

    def __getitem__(self, key):
        if isinstance( key, slice ) :
            #Get the start, stop, and step from the slice
            return [Vector3d.fromPointer (<uintptr_t> &(self.parent.body_points[i])) for i in xrange (*key.indices(len(self)))]
        else:
            return Vector3d.fromPointer (<uintptr_t> &(self.parent.body_points[key]))
    ...
    def __setitem__(self, key, value):
        if isinstance( key, slice ) :
            ...
            for i in xrange (*key.indices(len(self))):
                ...
                self.parent.body_points[i] = (<Vector3d> value[src_index]).thisptr[0]
                src_index = src_index + 1
        else:
            assert isinstance (value, Vector3d), "Invalid type! Expected Vector3d, but got " + str(type(value)) + "."
            self.parent.body_points[key] = (<Vector3d> value).thisptr[0]

    def __len__(self):
        return self.parent.body_points.size()

cdef class _InverseKinematicsConstraintSet_target_positions_Vector3d_VectorWrapper:
    cdef crbdl.InverseKinematicsConstraintSet *parent
    ...
    def __getitem__(self, key):
        if isinstance( key, slice ) :
            #Get the start, stop, and step from the slice
            return [Vector3d.fromPointer (<uintptr_t> &(self.parent.target_positions[i])) for i in xrange (*key.indices(len(self)))]
        else:
            return Vector3d.fromPointer (<uintptr_t> &(self.parent.target_positions[key]))
    ...
    def __setitem__(self, key, value):
        if isinstance( key, slice ) :
            ...
            for i in xrange (*key.indices(len(self))):
                assert isinstance (value[src_index], Vector3d), "Invalid type! Expected Vector3d, but got " + str(type(value[src_index])) + "."
                self.parent.target_positions[i] = (<Vector3d> value[src_index]).thisptr[0]
                src_index = src_index + 1
        else:
            assert isinstance (value, Vector3d), "Invalid type! Expected Vector3d, but got " + str(type(value)) + "."
            self.parent.target_positions[key] = (<Vector3d> value).thisptr[0]

    def __len__(self):
        return self.parent.target_positions.size()

cdef class _InverseKinematicsConstraintSet_target_orientations_Matrix3d_VectorWrapper:
    cdef crbdl.InverseKinematicsConstraintSet *parent
    ...
    def __getitem__(self, key):
        if isinstance( key, slice ) :
            #Get the start, stop, and step from the slice
            return [Matrix3d.fromPointer (<uintptr_t> &(self.parent.target_orientations[i])) for i in xrange (*key.indices(len(self)))]
        else:
            return Matrix3d.fromPointer (<uintptr_t> &(self.parent.target_orientations[key]))
    ...
    def __setitem__(self, key, value):
        if isinstance( key, slice ) :
            ...
            src_index = 0
            for i in xrange (*key.indices(len(self))):
                assert isinstance (value[src_index], Matrix3d), "Invalid type! Expected Matrix3d, but got " + str(type(value[src_index])) + "."
                self.parent.target_orientations[i] = (<Matrix3d> value[src_index]).thisptr[0]
                src_index = src_index + 1
        else:
            assert isinstance (value, Matrix3d), "Invalid type! Expected Matrix3d, but got " + str(type(value)) + "."
            self.parent.target_orientations[key] = (<Matrix3d> value).thisptr[0]

    def __len__(self):
        return self.parent.target_orientations.size()


cdef class InverseKinematicsConstraintSet:
    cdef crbdl.InverseKinematicsConstraintSet *thisptr
    cdef _InverseKinematicsConstraintSet_body_points_Vector3d_VectorWrapper body_points
    cdef _InverseKinematicsConstraintSet_target_positions_Vector3d_VectorWrapper target_positions
    cdef _InverseKinematicsConstraintSet_target_orientations_Matrix3d_VectorWrapper target_orientations

    def __cinit__(self):
        self.thisptr = new crbdl.InverseKinematicsConstraintSet()
        self.body_points = _InverseKinematicsConstraintSet_body_points_Vector3d_VectorWrapper (<uintptr_t> self.thisptr)
        self.target_positions = _InverseKinematicsConstraintSet_target_positions_Vector3d_VectorWrapper (<uintptr_t> self.thisptr)
        self.target_orientations = _InverseKinematicsConstraintSet_target_orientations_Matrix3d_VectorWrapper (<uintptr_t> self.thisptr)
    ...
    property body_points:
        def __get__ (self):
            vector_size = self.thisptr.body_points.size()
            prop = [Vector3d (address=<uintptr_t> &(self.thisptr.body_points[i])) for i in range(vector_size)]
            return prop

    property target_positions:
        def __get__ (self):
            vector_size = self.thisptr.target_positions.size()
            prop = [Vector3d (address=<uintptr_t> &(self.thisptr.target_positions[i])) for i in range(vector_size)]
            return prop

    property target_orientations:
        def __get__ (self):
            vector_size = self.thisptr.target_orientations.size()
            prop = [Matrix3d (address=<uintptr_t> &(self.thisptr.target_orientations[i])) for i in range(vector_size)]
            return prop
